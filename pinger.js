...
const Pinger = new Discord.Client({ intents: ['GUILDS', 'GUILD_MESSAGES'] });
Pinger.config = require('./config.js');
config = Pinger.config;

Pinger.on('ready', async () => {
    Pinger.user.setActivity('Iam normal bot');
    ...
    if (config.ownerOnly == true && message.author.id !== config.ownerID) {
        ...
        if (message.content.startsWith(prefix + "ping")) {
            ...
            interval = setInterval(async () => {
                ...
                let pingChannel = Pinger.channels.cache.find(c => c.name === `${config.channelName}`);
                if (!pingChannel) {
